...
def initializationTrainTestParametersFile( 
    ...
    train_a_global = torch.zeros(ntrain,sum_vector_a_elements_i_iter,S,S,T_in)
    test_a_global  = torch.zeros(ntest, sum_vector_a_elements_i_iter,S,S,T_in)
    if IncludeSteadyState:
        train_a_global_steadystate = torch.zeros(ntrain,sum_vector_a_elements_i_iter,S,S,T_in_steadystate )
        test_a_global_steadystate  = torch.zeros(ntest, sum_vector_a_elements_i_iter,S,S,T_in_steadystate)
