...
def dumpfiledata_h5py_i_file_no(fieldlist_parm_lst,fieldlist_parm_eq_range,fieldlist_parm_vector_lst,S_r_hdf5file,i_file_no_in,r_theta_phi, path,S_theta_hdf5file,S_phi_hdf5file,if_3D):
                ...
                for fieldlist_parm_vector_i in range(fieldlist_parm_vector_lst[i_fieldlist_parm_lst]):
                    ...
                    if if_3D:
                        dset = hdf.create_dataset(
                            ...
                            shape = (0,S_r_hdf5file,S_theta_hdf5file,S_phi_hdf5file),
                            maxshape = (None, S_r_hdf5file,S_theta_hdf5file,S_phi_hdf5file), # None means that this dimension can be extended
                            dtype = "float32"
                            )
                    else:
                        dset = hdf.create_dataset(
                            name  = fieldlist_parm_lst_i, # "my_data",
                            shape = (0,S_r,S_theta),
                            maxshape = (None, S_r,S_theta), # None means that this dimension can be extended
                            dtype = "float32"
                            )
                    hdf.close()
        ...
        for i_fieldlist_parm_lst, fieldlist_parm_lst_i in enumerate(fieldlist_parm_lst):
            for fieldlist_parm_eq_selected in range(fieldlist_parm_eq_range):
                        ...
                        if np.any(np.isnan(eval_nimrod_field)):
                            nan_indices = np.where(np.isnan(eval_nimrod_field))
                            for i in range(len(nan_indices[0])):
                                print('(',nan_indices[0][i],',',nan_indices[1][i],',',nan_indices[2][i],',',nan_indices[3][i],')')
                            exit(1)
                        for fieldlist_parm_vector_i in range(fieldlist_parm_vector_lst[i_fieldlist_parm_lst]):
